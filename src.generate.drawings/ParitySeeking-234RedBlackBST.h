...
private:
	...
	*/
	void put_bottom_up_pass(Node* x) {
		...
		while (x->parent->color == RED) {			// x->color is certainly RED
			...
			if (p == g->left) {
				if (g->right->color == RED) {		// Rule (a)
					deficiecyDirection = x == x->parent->left ? 'L' : 'R';
					drawTreeAndSave("Rule (a)", x->key, x->parent, deficiecyDirection);
					colorFlip(g);
					x = g;
				}
				else {
					if (x == p->right) {			// Rule (b)
						deficiecyDirection = x == x->parent->left ? 'L' : 'R';
						drawTreeAndSave("Rule (b)", x->key, x->parent, deficiecyDirection);
						x = p;
						rotateLeft(p);
					}
					deficiecyDirection = x == x->parent->left ? 'L' : 'R';
					drawTreeAndSave("Rule (c)", x->key, x->parent, deficiecyDirection);
		...
		{
			deficiecyDirection = x == x->parent->left ? 'L' : 'R';
			drawTreeAndSave("Rule (d)", x->key, x->parent, deficiecyDirection);
...
private:
	...
	void applyFixingRules(Node* & x, Node* y, Node* p) {
		...
		if (x == x->parent->left) {
			if (y->right->color == BLACK) 	// Fixing rule (a)
			{
			...
			if (y->left->color == BLACK)	// Fixing rule (a)
			{
				drawTreeAndSave("Fixing Rule (a)", y->key, p->parent, deficiecyDirection);
				rotateLeft(y);
			}
