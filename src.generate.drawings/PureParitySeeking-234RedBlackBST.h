...
private:
	...
	void put_bottom_up_pass(Node* x) {
		...
		while (x->parent->color == RED) {			// x->color is certainly BLACK
			...
			if (p == g->left) {
				if (g->right->color == RED) {		// Rule (c)
					...
					drawTreeAndSave("Rule (c)", x->key, p, overfullDirection);
					x->color = RED;
					g->left->color = BLACK;
					g->right->color = BLACK;
					x = g;
					...
					if (x == p->right) {			// Rule (b.2)
						overfullDirection = x == x->parent->left ? 'L' : 'R';
						drawTreeAndSave("Rule (b.2)", x->key, p, overfullDirection);
						x = p;
						rotateLeft(p);
					}
					overfullDirection = x == x->parent->left ? 'L' : 'R';
					drawTreeAndSave("Rule (b.1)", x->key, p, overfullDirection);
		...
		{
			overfullDirection = x == x->parent->left ? 'L' : 'R';
			drawTreeAndSave("Rule (a)", x->key, x->parent, overfullDirection);
			x->color = RED;							// Rule (a)
		} 
		else
		{
